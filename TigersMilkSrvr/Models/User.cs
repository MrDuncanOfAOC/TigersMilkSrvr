...
namespace TigersMilkSrvr.Models
{
    ...
    public class UserHelper
    {
        ...
        public static User Authenticate(string username, string password)
        {
            ...
            {
                conn.Open();
                cmd.CommandText = "(select AOCPORT.xxaoc_crypto_util_pkg.sf_authenticate_pwd_str((select upper(user_name) from TM_USERS where upper(email_address) = '" + username.ToUpper() + "'), '" + password + "') valid from dual)";

                if (cmd.ExecuteScalar().ToString().Equals("TRUE", StringComparison.CurrentCultureIgnoreCase))
                {
                    ...
                    string sql = "SELECT U.*, R.role_desc, UT.type_desc " +
                                 ...
                                 "   tm_user_type UT " +
                                 "WHERE(1 = 1) " +
                                 "      AND UPPER (email_address) = '" + username.ToUpper() + "' " +
